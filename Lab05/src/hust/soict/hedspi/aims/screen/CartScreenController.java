...
public class CartScreenController {
    ...
    public void initialize() {
    	...
    	FilteredList<Media> filteredData = new FilteredList<>(cart.getItemsOrdered(), s -> true);
    ...
    void btnPlayPressed(ActionEvent event) {
		...
		if (m instanceof DigitalVideoDisc) { // first is DVD
			DigitalVideoDisc m1 = (DigitalVideoDisc)m; 
			try {
		...
		else if (m instanceof CompactDisc) { // first is CD
			CompactDisc m1 = (CompactDisc)m; 
			try {
				d.setContentText(m1.play());
			} catch (PlayerException e) {
				// TODO Auto-generated catch block
    ...
    void btnRemovePressed(ActionEvent event) {
    	...
    	TotalCost.setText("" + this.cart.totalCost() + " $"); // Update totalCost
    }
}